<template>
  <v-card class="mt-4" v-if="todo">
    <v-card-actions class="px-2">
      ...
      <v-spacer></v-spacer>
      <v-btn v-if="!editMode" flat icon color="blue" @click="setEditMode(todo.title)">
      ...
      <v-btn color="red lighten-3" flat icon @click="load(deleteTodoShow, todo)">
        <v-icon>close</v-icon>
      </v-btn>
    </v-card-actions>
    <v-divider></v-divider>
    <v-card-title class="px-2 subheading blue--text">
      ...
      <v-layout v-if="!editMode" align-center justify-start @dblclick="setEditMode(todo.title)">
        <span :class="todo.completed && 'completed' || 'text--lighten-2'">{{ todo.title }}</span>
      </v-layout>
      <v-text-field
        v-else
        v-model="editTitle"
